...
<?php global $wp_query; $total_pages = $wp_query->max_num_pages; if ( $total_pages > 1 ) { ?>
                <div id="nav-above" class="navigation">
                    <div class="nav-previous"><?php next_posts_link(__( '<span class="meta-nav">&laquo;</span> Older posts', 'mazaltov' )) ?></div>
                    <div class="nav-next"><?php previous_posts_link(__( 'Newer posts <span class="meta-nav">&raquo;</span>', 'mazaltov' )) ?></div>
                </div><!-- #nav-above -->
<?php } ?>
...
<?php the_content( __( 'Continue reading <span class="meta-nav">&raquo;</span>', 'mazaltov' )  ); ?>
<?php wp_link_pages('before=<div class="page-link">' . __( 'Pages:', 'mazaltov' ) . '&after=</div>') ?>
                    </div><!-- .entry-summary -->
...
<?php else : ?>
                ...
                <div id="post-0" class="post no-results not-found">
                    <h2 class="entry-title"><?php _e( 'Nothing Found', 'mazaltov' ) ?></h2>
                    <div class="entry-content">
                        <p><?php _e( 'Sorry, but nothing matched your search criteria. Please try again with some different keywords.', 'mazaltov' ); ?></p>
    <?php get_search_form(); ?>
                    </div><!-- .entry-content -->
                </div>
